...
import { useBorrowingApi } from "@/presentation/logics/app/borrowings";
...
const DataBorrowings = () => {
  ...
  const { useAllBorrowings, useBorrowingById } = useBorrowingApi();
  ...
  const { data, isPending, isError, error } = useAllBorrowings();
...
function AddBook({ visible, setVisible }) {
  const { getItem } = useSecureLS();
  const user = getItem("user");
  const { createBorrowing } = useBorrowingApi(user);
